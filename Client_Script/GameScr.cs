...
public class GameScr : mScreen, IChatable
{
    ...
    public Dictionary<int, ItemOptionTemplate> iOptionTemplates;
    ...
    public void onOSkill(sbyte[] oSkillID)
    {
        ...
        {
            Debug.LogError("I:" + i);
            Debug.LogError("oSkillID[i]:" + oSkillID[i]);
            try
            {
    ...
    public void onKSkill(sbyte[] kSkillID)
    {
        ...
        for (int i = 0; i < kSkillID.Length; i++)
        {
            ...
            for (int j = 0; j < Char.myCharz().vSkill.size(); j++)
            {
                ...
                {
                    keySkill[i] = onScreenSkill[i] = skill;
                    break;
                }
            }
        }
        ...
        try
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                for (int j = 0; j < parts[i].pi.Length; j++)
                {
                    ...
                    sb.AppendLine($"[{parts[i].pi[j].id},{parts[i].pi[j].dx},{parts[i].pi[j].dy}]");
                }
            }

            //File.WriteAllText("partData.txt", sb.ToString());
        }
        catch (Exception ex)
        {
    ...
    public static void setSkillBarPosition()
    {
        ...
        else
        {
            ...
            Skill[] array = GameScr.onScreenSkill;
            ...
            int baseX = (int)(2.5 * (double)wSkill);
            for (int k = 0; k < xS.Length; k++)
            {
                xS[k] = k * wSkill + baseX;
                yS[k] = ySkill;
                if (xS.Length > 5 && k >= xS.Length / 2)
                {
                    xS[k] = (k - xS.Length / 2) * wSkill + baseX;
                    yS[k] = ySkill - 32;
                }
            }
        }
    }
    public static int yClick = GameScr.ySkill - 32;
    ...
    public void playerMenu(Char c)
    {
        ...
        if (Char.myCharz().cPower < 100_000_000_000L && Char.myCharz().charFocus.cPower < 100_000_000_000L)
        {
            vPlayerMenu.addElement(new Command(mResources.CHAR_ORDER[8], 110392));
        }
    ...
    public override void updateKey()
    {
        ...
        if (GameCanvas.isTouch && !ChatTextField.gI().isShow && !GameCanvas.menu.showMenu)
        {
            ...
            MainMod.gI().UpdateKey();
        }
        checkAuto();
        //NQMP.NQMPMain.UpdateKey();
    ...
    public void doUseHP()
    {
        ...
        long num = mSystem.currentTimeMillis();
        if (num - lastUsePotion >= 10000 && Char.myCharz().isHavePean())
        {
    ...
    public override void update()
    {
        ...
        try
        {
            ...
            {
                long num = (dHP - Char.myCharz().cHP) / 2;
                if (num < 1)
                {
                    num = 1;
                }
            ...
            {
                long num2 = (dMP - Char.myCharz().cMP) / 2;
                if (num2 < 1)
                {
                    num2 = 1;
                }
    ...
    public static Mob findMobInMap2(int mobId)
    {
        for (int i = 0; i < vMob.size(); i++)
        {
            Mob mob = (Mob)vMob.elementAt(i);
    ...
    private void paintImageBar(mGraphics g, bool isLeft, Char c)
    {
        if (c != null)
        {
            ...
            else
            {
                ...
                num3 = (int)(c.cHP * hpBarW / c.cHPFull);
                num4 = (int)(c.perCentMp * mpBarW / 100);
            }
                ...
                try
                {
                    ...
                    for (int i = 0; i < num4; i++)
                    {
                        try
                        {
                            ...
                            if (skill == null)
                            {
                                mFont.tahoma_7b_dark.drawString(g, i.ToString(), xSkill + xS[i] + 13, yS[i] + 8, mFont.CENTER);
                                continue;
                            }
                            if (skill == Char.myCharz().myskill)
                            {
    ...
    private void actDead()
    {
        ...
        myVector.addElement(new Command(AutoNhat.isNRDMap(TileMap.mapID) ? "Hồi sinh\nngay 100tr vàng" : mResources.DIES[3], 110383));
        GameCanvas.menu.startAt(myVector, 3);
    }
    ...
    public void actionPerform(int idAction, object p)
    {
        ...
        switch (idAction)
        {
            ...
            case 110391:
                ...
                break;
            case 110392:
                Service.gI().clanInvite(3, Char.myCharz().charFocus.charID, -1, -1);
                break;
            case 12005:
    ...
    public void chatVip(string chatVip)
    {
        ...
        if (chatVip.StartsWith("BOSS"))
        {
            try
            {
                ...
                {
                    AutoPro.instance.chatVips.RemoveAt(0);
                }
            }
            catch { }
        }
