...
public class Service
{
    ...
    public void androidPack()
    {
        ...
        try
        {
            ...
            message.writer().writeUTF(SystemInfo.deviceUniqueIdentifier);
            session.sendMessage(message);
        }
        catch (Exception ex)
        {
    ...
    public void setClientType()
    {
        ...
        try
        {
            ...
            message.writer().writeInt(Screen.width);
            message.writer().writeInt(Screen.height);
            ...
            message.writer().writeUTF(GameMidlet.VERSION);
            ...
            else if (vMob.size() > 0)
            {
                ...
                for (int k = 0; k < vMob.size(); k++)
                {
                    ...
                    if (!mob2.isMobMe)
                    {
                        ...
                        message.writer().writeInt(mob2.templateId);
                        continue;
                    }
    ...
    {
        if (MainMod.gI().Chat(text)) return;
        Message message = null;
        try
        {
            message = new Message((sbyte)44);
    ...
    public void kigui(sbyte action, int itemId, int quantity, int price)
    {
        Message message = null;
        try
        {
            ...
            if (action == 0)
            {
                message.writer().writeByte(itemId); // index
                message.writer().writeInt(quantity);
    ...
    public void changeOnKeyScr(sbyte[] skill)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                catch { message.writer().writeByte(-1); }
            }
            session.sendMessage(message);
        }
        catch (Exception ex)
    ...
    public long lastRequestPing = mSystem.currentTimeMillis();
    public void pingToServer()
    {
        Message message = null;
        try
        {
            message = new Message(-109);
            message.writer().writeByte(7);
            lastPing = mSystem.currentTimeMillis();
            lastRequestPing = mSystem.currentTimeMillis();
            session.sendMessage(message);
        }
        catch (Exception ex)
        {
