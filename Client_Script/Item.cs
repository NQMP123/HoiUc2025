...
public class Item
{
    ...
    public string getPrice()
    {
        ...
        if (buyGold <= 0 && buyDiamond <= 0)
        {
            return null;
        }
        if (buyGold > 0 && buyDiamond <= 0)
        {
            result = buyGold + mResources.XU;
        }
        else if (buyDiamond > 0 && buyGold <= 0)
        {
            result = buyDiamond + mResources.LUONG;
        }
        else if (buyGold > 0 && buyDiamond > 0)
        {
            result = buyGold + mResources.XU + "/" + buyDiamond + mResources.LUONG;
        }
        return result;
    }
    ...
    public static List<int> listTypeBody = new List<int>();
    public bool isTypeBody()
    {
        if ((0 <= template.type && template.type < 6) || template.type == 32 || template.type == 36
            || template.type == 11 || template.type == 23 || template.type == 24 || template.type == 19 || template.type == 26 || template.type == 18 || template.type == 23 || template.type == 17)
        {
            return true;
        }
        if (listTypeBody.Contains(this.template.type))
        {
            return true;
        }
        return false;
