...
{
    public class AutoTrainVIP : IActionListener, IChatable
    {
        private static AutoTrainVIP _Instance;
        ...
        public static AutoTrainVIP getInstance()
        {
            if (_Instance == null)
                _Instance = new AutoTrainVIP();
            return _Instance;
        }
        ...
        private static Mob GetNextMob(int type)
        {
            ...
            for (int i = 0; i < GameScr.vMob.size(); i++)
            {
                ...
                {
                    if (mob2.levelBoss != 0 && !AutoTrainVIP.isAvoidSuperMob)
                    {
                        continue;
                    }
                    if (AutoTrainVIP.listMobIds.Contains(mob2.mobId))
                    {
                        return mob2;
                    }
                }
